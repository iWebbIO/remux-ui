...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/iWebbIO/remux-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/iWebbIO/remux-ui/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_dev() {
    ...
    XUI_UPDATE_TAG="dev-latest" bash <(curl -Ls https://raw.githubusercontent.com/iWebbIO/remux-ui/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    if replace_xui_script "https://raw.githubusercontent.com/iWebbIO/remux-ui/main/x-ui.sh" "false"; then
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/iWebbIO/remux-ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/iWebbIO/remux-ui/master/install.sh)${plain}"
    echo ""
...
update_shell() {
    if replace_xui_script "https://github.com/iWebbIO/remux-ui/raw/main/x-ui.sh" "true"; then
